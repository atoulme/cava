...
final class PacketHeader {
  ...
  static PacketHeader decode(Bytes data) {
    ...
    Signature signature = Signature.create(encodedSignature);
    Bytes signedPayload = data.slice(97, data.size() - 97);
    byte typeByte = data.get(97);
    Bytes payloadBytes = data.slice(98);
    PublicKey publicKey;
    try {
      publicKey = PublicKey.recoverFromSignature(signedPayload, signature);
    ...
    Bytes computedHash =
        Hash.keccak256(Bytes.wrap(Bytes.wrap(signature.encodedBytes(), Bytes.of(typeByte)), payloadBytes));
  ...
  PacketHeader(KeyPair keyPair, byte packetType, Bytes payload) {
    ...
    this.hash = Hash.keccak256(Bytes.wrap(signature.encodedBytes(), typeBytes, payload));
