...
public final class VertxDiscoveryService {
   ...
   * @param advertisedAddress the address advertised as part of the endpoint
   ...
   * @param logger logger to use for all logging.
   */
  @VisibleForTesting
   ...
   * @param wireProtocolPort the port to advertise as
   * @param logger logger to use for all logging.
   * @param timeSupplier supplier of time
   */
  @VisibleForTesting
  VertxDiscoveryService(
      Vertx vertx,
      PeerRepository peerRepository,
      List<String> bootstrapPeers,
      InetSocketAddress listenAddress,
    ...
    checkArgument(advertisedAddress != null);
  ...
  void sendPacket(Endpoint to, Packet packet) {
    logger.debug("Sending packet {} to {}", packet, to);
    socket.send(Buffer.buffer(packet.toBytes().toArrayUnsafe()), to.udpPort(), to.address(), ar -> {
      if (ar.failed()) {
  ...
  void receivePacket(DatagramPacket datagram) {
    ...
    logger.debug("Receiving packet from {}", datagram.sender());
  ...
  public AsyncCompletion start() {
    ...
    vertx.createDatagramSocket(new DatagramSocketOptions()).listen(listenPort, listenHost, res -> {
      ...
      Endpoint thisEndpoint =
          new Endpoint(advertisedAddress.getHostString(), advertisedAddress.getPort(), wireProtocolPort);
